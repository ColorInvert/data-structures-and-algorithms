...
//? Partially adapted from a response in Stack Overflow. "EDIT: David Thomas suggests a nice alternative: /^[\w.]+$/i -- possible locale issues."
...
const validateEmail = (email) => {
  ...
  //check if the name of the person, and the name of the website are valid.
  nameGood = /(^[\w]+@[\w]+\.)|(^[\w]+\.[\w]+@[\w]+\.)/.test(email);

  //check if the toplevel domain is one of the allowed ones.
  domainGood = /com$|net$|org$/.test(email);

  if (nameGood === true && domainGood === true) {
    return true;
  }
...
const validatePhoneNumber = (phoneNumber) => {
  return /^([0-9]{3}|\([0-9]{3}\))[ -]?[0-9]{3}[ -]?[0-9]{4}$/mg.test(phoneNumber);
};
