...
const updateAnimal = (arr, callback) => {
  ...
  //Define, then map updatedAnimals using the callback function. Return.
  var updatedAnimals = [];

  updatedAnimals = arr.map(item => callback(item));
...
//This is the default behavior of argument-less sort. Sort(), return.
...
const sortNumbers = (arr) => {
  ...
  //Define sortedNums array, and sort by subtracting item a by item b. If it's a positive number, move one direction, if negative, other. Return sorted array.
  var sortedNums = [];

  sortedNums = arr.sort((a, b) => { return a - b; });
...
const sortBackwards = (arr) => {
  ...
  //Exact opposite of above. Define sortedNums array, and sort by subtracting item b by item a. If it's a positive number, move one direction, if negative, other. Return sorted array.
  backSortedNums = arr.sort((a, b) => { return b - a; });

  return backSortedNums;
...
//This is default behavior of sort with no arguments. sort(), return.
...
const sortByPrice = (arr) => {
  ...
  //define new array, and for each object, check it's price value, and subtract it against it's... comparator? is that a word? Anyway, return that.

  var costs = [];

  costs = arr.sort((a, b) => { return a.price - b.price; });
...
const alphabetizeBetter = (arr) => {
  ...
  alphabetical = arr.sort((a, b) => {

    //make a temp variable that is the 2 elements being compared in lowercase.
    const tempA = a.toLowerCase();
    const tempB = b.toLowerCase();

    //compare the lowercased versions of A and B, and sort the original version of A and B based on the results. Return.
    const returnVar = tempA > tempB ? 1 : -1;
...
const sortByLength = (arr) => {
  ...
  strLength = arr.sort((a, b) => {

    //make a temporary variable that is the length value of A and B, then compare those, and sort A and B based on result. Return.
    const tempA = a.length;
    const tempB = b.length;
...
const sortNumbersByLength = (arr) => {
  ...
  numsLength = arr.sort((a, b) => {

    //make temporary variables that are the stringified, and then length counted versions of A and B, compare those, and sort A and B based on result. Return.
    const tempA = (a.toString()).length;
    const tempB = (b.toString()).length;
