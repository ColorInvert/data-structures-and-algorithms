...
const toLastNames = people => {
  ...
  fullnames = people.map((x) => `${x.firstName} ${x.lastName}`);
...
const addValues = (arr) => {
  ...
  //define distilledValues, which is array.reduce()'d, with accumulator + current value, starting at initial value, then return.
  var distilledValues = arr.reduce((acc, curr) => acc + curr, initial);
...
const addPurchases = (arr) => {
  ...
  //define total cost as result of the accumulated purchasePrice of each object in the array, then return accumulated cost.
  var totalCost = arr.reduce((acc, curr) => acc + curr.purchasePrice, initial);
...
const countNumberOfElements = (arr) => {
  ...
  //ok this is hacky. acc here is constantly reset to the index value of our reduce function, plus 1. Which means that whatever the length of the array, the reduce will terminate and leave acc at the final index, plus one to compensate for indexes starting at 0. Return.
  var elementCount = arr.reduce((acc, currVal, currInd) => acc = currInd + 1, initial);
...
const returnNames = (arr) => {
  ...
  arr.reduce((acc, curr) => names.push(curr.name), []);
...
const reversedString = (str) => {
  ...
  //define/transform our string into an array of characters
  var stringArray = str.split('');

  //define an array called reversed
  var reversed = [];

  //reduce our character array, where each new index of our array is unshifted to the front. This reverses stringArray, and saves it into reversed to give us our original string turned into an array, and then turned into a reversed array.
  stringArray.reduce((acc, curr) => reversed.unshift(curr), '');


  //prepare to return the reversed array, but we need a string for the requirement, not array. So perform a final join command, with the argument '' to ensure no commas.
