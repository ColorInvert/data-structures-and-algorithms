...
const screenForNames = (arr) => {
  return arr.filter(i => /^(Mr\.|Mrs\.|Ms\.|Dr\.)( {1}\w)/.test(i));
};
...
//? Thanks, Flexiple. https://flexiple.com/javascript/javascript-capitalize-first-letter/
...
let biggerThanLuke = (arr) => {
  ...
  biggerArray = arr.filter(i => Number(i.mass) > targetMass);

  namesOnlyArray = biggerArray.map(i => i.name);
...
//? this one seems odd.
const isSecure = (url) => {
  var secureURL = 'https://secure.com';
...
const detectTicTacToeWin = (board) => {
  ...
  //Definitions for the left column, middle column, and right column.
  var lineA = [board[0][0], board[1][0], board[2][0]];
  var lineB = [board[0][1], board[1][1], board[2][1]];
  var lineC = [board[0][2], board[1][2], board[2][2]];

  //definitions for the top row, middle row, and bottom row.
  var lineD = [board[0][0], board[0][1], board[0][2]];
  var lineE = [board[1][0], board[1][1], board[1][2]];
  var lineF = [board[2][0], board[2][1], board[2][2]];

  //Definition for TopToBottom diagonal, and BottomToTop diagonal
  var lineG = [board[0][0], board[1][1], board[2][2]];
  var lineH = [board[2][0], board[1][1], board[0][2]];


  //an array that consists of all of the above line definitions
  var testingArray = [lineA, lineB, lineC, lineD, lineE, lineF, lineG, lineH];


  //Check if all array elements all match each other, and check if all array elements are all non-empty strings. if so, return true.
  function lineTester(arr) {

    if (arr[0] === arr[1] && arr[1] === arr[2] && arr[0] === arr[2] && arr[0] !== '' && arr[1] !== '' && arr[2] !== '') {
      //if the currently tested line survives this conditional, we have a tic tac toe! Set our latch to true to state the board has one.
      winLatch = true;
    }

  }

  //run the function lineTester against every line defined above. This ultimately checks the entire board for any tic tac toe wins, and flips the winLatch to true if 1 or more are found.
  testingArray.map(line => lineTester(line));

  //did our winLatch ever get tripped? return the state of it.
  return winLatch;
};
