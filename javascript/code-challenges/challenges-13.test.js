...
const longestString = (arr) => {
  ...
  //Handle 0 length arrays by returning a -1.
...
const standardizePhoneNumbers = (arr) => {
  ...
  arr.map(x => {
    ...
    standardized = x.replace(reg, '');
    standardArr.push(standardized);
...
const onlyOddChars = (str) => {
  ...
  for (let i = 0; i < str.length; i++) {
    ...
    if (i % 2 !== 0) {
      oddArr.push(str.charAt(i));
    }
  }
  var oddString = oddArr.join('');
  return oddString;
...
const unenrollBrook = (arr) => {
  ...
  return arr.map(i => {
    ...
    return i.filter(j => !j.includes(target));
