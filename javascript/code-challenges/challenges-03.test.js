...
const addTwo = (arr) => {
  ...
  //push to new array element += 2 for each element, then return.
  arr.forEach(item => {
    plusTwo.push(item += 2);
  });
...
const typeNum = (arr) => {
  ...
  //perform typeof for each element, and if we get the result 'number', add it to the  new array, and return.
  onlyNum = arr.filter(arr => typeof arr === 'number');
...
const containsAnd = (arr) => {
  ...
  //perform .includes for each element, to check if 'and' is present anywhere. if so, add element to new array, and return.
  andHavers = arr.filter(arr => arr.includes('and'));
...
const oddValues = (arr) => {
  ...
  oddsOnly = arr.filter(arr => (arr % 2 !== 0));
...
const notInFirstArray = (forbiddenValues, arr) => {
  ...
  //compare every element of arr with every element of forbiddenValues, and add to the new array each item that is !NOT! present in both, then return.
  exclusiveArr = arr.filter(arr => !forbiddenValues.includes(arr));

  return exclusiveArr;

  //? Shout outs to Stack Overflow user Jota3 for an elegant answer on sweeping through 2 arrays for matches. Answer is theirs, retooled to fit this context, plus a ! added to do the opposite.
  //? https://stackoverflow.com/revisions/46957719/2
