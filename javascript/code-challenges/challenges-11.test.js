...
function transformToLis(obj) {
  ...
  //Thanks to MDN Web Docs for this code snippet for Object.entries()
  for (const [key, value] of Object.entries(obj)) {
    listed.push(`<li>${key}: ${value}</li>`);
  }
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  return input.map(i => {
    ...
    return i.filter(element => element % 5 === 0 && typeof element === 'number').map(element => Math.pow(2, element));
...
let findShortest = (data) => {
  var shortestVal = 100000000;
  var shortestChar = '';

  data.map(i => {

    //THE HEIGHT VALUE IS A STRING, WHICH BREAKS COMPARISON IF NOT CONVERTED TO A NUMBER.
    if (Number(i.height) < shortestVal) {
      shortestChar = i.name;
      shortestVal = i.height;
    }
