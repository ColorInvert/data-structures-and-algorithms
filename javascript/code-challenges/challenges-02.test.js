...
const forLoopTwoToThe = (arr) => {
  ...
  //loop over the input array, number of loops dictated by length
  for (let i = 0; i < arr.length; i++) {

    //at each index, redefine element as 2 to the power of (indexvalue), then return.
...
const forEachTwoToThe = (arr) => {
  ...
  //push to new array the result of 2 to the power of (input), then return.
  arr.forEach(item => {
    powered.push(Math.pow(2, item));
  });
...
const mapTwoToThe = (arr) => {
  ...
  //pass each element of arr down into map, 2 to the (element) power, return.
  var twoPower = arr.map(x => Math.pow(2, x));
  return twoPower;
...
const charCode = (arr) => {
  //Make new array, and use charcodeat to transform the 1 character input to it's char code, then return.
  var charred = arr.map(x => x.charCodeAt(0));
  return charred;
};
