...
const sortStarWarsCharacters = (starWarsArr) => {
  ...
  starWarsTallest = starWarsArr.sort((a, b) => { return b.height - a.height; });

  return starWarsTallest;
...
const joinArray = (arr) => {
  ...
  //note the space in the  VVV    join command, serving as a separator.
  var strungArr = arr.join(' ');
  return strungArr;
...
const howMuchPencil = (str) => {
  ...
  var shrinkingString = str;

  result.push(str);
  for (let i = 0; i < str.length; i++) {

    //loop, slicing one more letter off the front each time
    shrinkingString = shrinkingString.slice(1);

    //push into result each iteration of the for loop
    result.push(shrinkingString);
...
const wordsToCharList = (arr) => {
  ...
  //spread operator.
  var tastyCharredString = [...arr];

  return tastyCharredString;
...
const listFoods = (recipe) => {
  ...
  //! Every ingredient has 2 spaces before the name of the recipe item. start from after the second space to end of array element and you will get all needed items.

  //For loop, handles each of the items in Ingredients one at a time.
  for (let i = 0; i < recipe.ingredients.length; i++) {

    //define ing. This is what will be passed up to our results array, and is the name of the ingredient alone without any units.
    var ing = '';


    //ok, hear me out here.

    //Get the recipe item from the ingredients list our for loop is currently at, and slice it.

    //the args for slice are: the FIRST instance of ' ' (space) PAST the 4th string character. This skips every first space since the furthest in first space is at index 2.

    //However, if we just make ing equal that slice value described above, the second space itself will be included, giving ' gruffalo' instead of 'gruffalo', so we add 1 to the index of our slice location.
    ing = recipe.ingredients[i].slice(recipe.ingredients[i].indexOf(' ', 4) + 1);


    //push this loop's value of ing to the results array. I had considered turning this into a 1 liner by removing the use of ing, but I decided not to be *that* mean.
    result.push(ing);
  }


  //once the for loop's done, hand off the results array.
  return result;
};
